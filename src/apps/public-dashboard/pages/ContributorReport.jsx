...
import { apiDownloadFile, apiGet } from '../../../services/api';
...
const NoDataYet = () => (
  <Card>
    <div className="contrib-empty-state">
      ...
      >
        <span>Back to contributors</span><span aria-hidden="true">←</span>
      </Link>
    </div>
  </Card>
);
...
export default function ContributorOverview() {
  ...
  useEffect(() => {
    ...
    async function loadReport() {
      ...
      setError('');
      try {
        const data = await apiGet(`/api/admin/reports/contributors/${userId}`);
        if (!cancelled) {
          if (!data?.ok) {
            setError(data?.error || 'Failed to load contributor overview');
            setReport(null);
          } else {
            setReport(data);
          }
        ...
        if (!cancelled) {
          setError(err.message || 'Failed to load contributor overview');
          setReport(null);
        }
      } finally {
  ...
  async function handleExport() {
    ...
    try {
      await apiDownloadFile(
        `/api/admin/reports/contributors/${userId}/export?from=${exportRange.from}&to=${exportRange.to}`,
        `field-report-${exportRange.from}-to-${exportRange.to}.pdf`
      );
      setExportOpen(false);
    } catch (err) {
  ...
  if (error) return (
    <section>
      <div className="card" style={{ padding: '1.25rem', color: '#ef4444' }}>{error}</div>
      <Link to="/dashboard/contributors" className="btn-secondary" style={{ display: 'inline-flex', marginTop: '1rem' }}>
  ...
  const totalTokens  = stats?.totalTokens ?? stats?.totalPoints ?? 0;
    ...
    <section style={{ display:'flex', flexDirection:'column', gap:'1.25rem', paddingBottom:'2rem', fontFamily:'var(--font-sans)' }}>
      ...
      <div className="contributor-hero">
        ...
        <div className="contrib-hero-actions">
          <Link to="/dashboard/contributors" className="contributor-hero__cta">
            <span>All contributors</span><span aria-hidden="true">←</span>
          </Link>
        </div>
      </div>
        ...
        <Card className="contrib-export-panel">
          <div className="contrib-export-head">
            <div>
              ...
              <p className="contrib-export-sub">Download a PDF summary of this contributor's approved cleanups for a date range.</p>
            </div>
            <button type="button" className="contrib-export-close" aria-label="Close export panel" onClick={() => setExportOpen(false)}>✕</button>
          </div>
          <div className="contrib-export-row">
            <div className="contrib-export-field">
              ...
              <input id="export-from" type="date" className="contrib-export-input" value={exportRange.from} max={exportRange.to} onChange={(event) => setExportRange((range) => ({ ...range, from: event.target.value }))} />
            </div>
            <div className="contrib-export-field">
              <label htmlFor="export-to" className="contrib-export-label">To</label>
              <input id="export-to" type="date" className="contrib-export-input" value={exportRange.to} min={exportRange.from} max={toDateInputValue(new Date())} onChange={(event) => setExportRange((range) => ({ ...range, to: event.target.value }))} />
            </div>
            <button type="button" className="ma-cta" disabled={exporting} onClick={handleExport}>
              ...
              <div style={{ marginTop:'1rem' }}>
                ...
                {topBrands.length === 0 ? (
                  ...
                  topBrands.slice(0, 5).map((b) => (
                    <BarRow key={b.key} label={b.label} pct={Math.round((b.count / topBrandsMax) * 100)} valueLabel={`${b.count} reports`} color="#378add" />
                  ))
                )}
              </div>
            </Card>
                ...
                <div style={{ display:'flex', flexDirection:'column', marginBottom:'0.9rem' }}>
                  {habitatObservations.speciesSighted.map((s, i, arr) => (
                    <div key={s.key} className="contrib-list-row" style={{ borderBottom: i < arr.length - 1 ? '1px solid var(--border-light)' : 'none' }}>
                      ...
                      <strong style={{ fontSize:'0.78rem', color:'var(--text-muted)' }}>{s.count} sighting</strong>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div style={{ display:'flex', flexDirection:'column' }}>
                  {habitatObservations.habitatStress.map((h, i, arr) => (
                    <div key={h.key} className="contrib-list-row" style={{ borderBottom: i < arr.length - 1 ? '1px solid var(--border-light)' : 'none' }}>
                      ...
                      <strong style={{ fontSize:'0.78rem', color:'var(--text-muted)' }}>{h.count} notes</strong>
                    </div>
                  ))}
                </div>
              )}
            ...
            {myActivities.length > 5 && (
              <Link to="/dashboard/activities"
                style={{ marginTop:'0.75rem', width:'100%', background:'transparent',
                  ...
                  fontFamily:'var(--font-sans)', textDecoration:'none', justifyContent:'center' }}
