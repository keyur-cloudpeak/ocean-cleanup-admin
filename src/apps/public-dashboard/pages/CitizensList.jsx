...
export default function CitizensList() {
  ...
  const { citizens = [], orgMap, status, error } = useSelector((state) => state.users);
  ...
  const pageCount = Math.max(1, Math.ceil(filteredCitizens.length / pageSize));
  const currentPageCitizens = filteredCitizens.slice((page - 1) * pageSize, page * pageSize);
  ...
  return (
    <section>
      ...
      <div className="card mb-6" style={{
        ...
        <div>
          <h3 style={{ marginBottom: '0.25rem' }}>Citizens List</h3>
          <p className="text-muted" style={{ margin: 0 }}>
            All registered citizens on the platform.
          </p>
        </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <UserTable
          users={currentPageCitizens}
          loading={loading}
          emptyLabel="No citizens registered yet."
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 0.75rem' }}>
        <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
          Showing {currentPageCitizens.length} of {filteredCitizens.length} citizens
        </span>
