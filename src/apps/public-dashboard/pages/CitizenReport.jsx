...
const NoDataYet = () => (
  <div className="co-panel">
    <div className="co-empty-state">
      ...
      <Link to="/dashboard/citizens" id="citizen-submit-empty" className="co-cta" style={{ alignSelf: 'center', marginBottom: 0, marginTop: '0.4rem', display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
        <span>Back to citizens</span><span aria-hidden="true">←</span>
      </Link>
    </div>
  </div>
);
...
export default function CitizenOverview() {
  ...
  useEffect(() => {
    ...
    async function loadReport() {
      ...
      setError('');
      try {
        const data = await apiGet(`/api/admin/reports/citizens/${userId}`);
        if (!cancelled) {
          if (!data?.ok) {
            setError(data?.error || 'Failed to load citizen overview');
            setReport(null);
          } else {
            setReport(data);
          }
        ...
        if (!cancelled) {
          setError(err.message || 'Failed to load citizen overview');
          setReport(null);
        }
      } finally {
  ...
  if (error) return (
    <section>
      ...
      <Link to="/dashboard/citizens" className="btn-secondary" style={{ display: 'inline-flex', marginTop: '1rem' }}>
        Back to citizens
      </Link>
    </section>
  );
  ...
  const leaderboard = report?.leaderboard?.leaderboard || [];
  ...
  return (
    <div className="co-root">
      ...
      <div className="co-hero">
        ...
        <Link to="/dashboard/citizens" id="citizen-submit-hero" className="co-cta">
          <span>All citizens</span><span aria-hidden="true">←</span>
        </Link>
      </div>
